...
def load_crf_config(file_path):
    """
    Load conifg from file. Eg: {'c1': 1.0, 'c2': 0.001, ...}
...
class CrfTokenizer(BaseTokenizer):
    def __init__(self, config_root_path="", bi_grams_path="bi_grams.txt", tri_grams_path="tri_grams.txt", crf_config_path="crf_config.txt", features_path="crf_features.txt", model_path="vi-segmentation.crfsuite", load_data_f_file=load_data_from_dir, base_lib="sklearn_crfsuite"):
        """
        Initial config
    ...
    def _check_bi_gram(self, a, relative_id):
        ...
        param relative_id: relative id compare to current word. Eg: -1 = previous word, +1 = next word
    ...
    def _check_tri_gram(self, b, relative_id):
        ...
        param b: list of words [word2, word1, word]. Note: word2 can be next next word or previous previous word but same format
        param relative_id: relative id compare to current word. Eg: -2 = previous previous word, +2 = next next word
    ...
    def _get_base_features(self, features_cfg_arr, word_list, relative_id=0):
        ...
        param feature_cfg_arr: array of feature names
        param word_list: related word list, word word+1, ...
        param relative_id: relative id compare to target word. Eg: -2,-1,0,+1,+2
    ...
    def create_sentence_features(self, prepared_sentence):
        ...
        param prepared_sentence: input sentence. Eg: ['Hello','World'] is a sentence = list of words
        
        return: List of features. Eg: [{'bias': 1.0, 'lower': 'hello'}, {'bias': 1.0, 'lower':'world'}]
        """
    ...
    def prepare_training_data(self, sentences, labels):
        ...
        return: Array of sentence, sentence is now a list of feature dictionary, each dictionary is correspongding to a word
        """
        X = []
        y = []
    ...
    def train(self, data_path):
        """
        Train data loaded from file and save model to model_path
        ...
        if self.base_lib == "sklearn_crfsuite":
            crf = sklearn_crfsuite.CRF(algorithm=self.crf_config['algorithm'], c1=self.crf_config['c1'], c2=self.crf_config['c2'], max_iterations=self.crf_config['max_iterations'], all_possible_transitions=self.crf_config['all_possible_transitions'])
            crf.fit(X, y)
