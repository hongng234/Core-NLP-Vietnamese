...
class LongMatchingTokenizer(BaseTokenizer):
    def __init__(self, bi_grams_path='bi_grams.txt', tri_grams_path='tri_grams.txt'):
        """
        Initial config
        param bi_grams_path: path to bi_grams set
        param tri_grams_path: path to tri_grams set
        """
...
def test():
    ...
    tokens = lm_tokenizer.tokenize("Các vấn đề liên quan đến thuế nói chung và thuế thu nhập cá nhân nói riêng đều rất phức tạp. Để giúp bạn đọc dễ dàng hơn khi tính thuế, bài viết dưới đây sẽ trình bày chi tiết cách tính thuế thu nhập cá nhân mới nhất đối với các khoản thu nhập từ tiền lương, tiền công.")
    print(tokens)
