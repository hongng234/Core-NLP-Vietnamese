...
class KerasTextClassifier(BaseTextClassifier):
    def __init__(self, tokenizer, word2vec, model_path, max_length=20, n_epochs=15, batch_size=6, n_class=2, sym_dict=None):
        """
    ...
    def word_embed_sentences(self, sentences, max_length=20):
        ...
        for sent in sentences:
            ...
            if len(embed_sent) > max_length:
                ...
                embed_sent = np.concatenate((embed_sent, np.zeros(shape=(max_length - len(embed_sent), self.word_dim), dtype=float)), axis=0)
    ...
    def tokenize_sentences(self, sentences):
        ...
        return token_list, max_length

    
    @staticmethod
    def load_data_from_file(file_path):
        """
        Method to laod sentences from file
...
def test():
    ...
    from word2vec_gensim import Word2Vec
    ...
    keras_text_classifier = BiDirectionalLSTMClassifier(tokenizer=tokenizer, word2vec=word2vec_model.wv, model_path='../models/sentiment_model.h5', max_length=10, n_epochs=10, sym_dict=sym_dict)

    X, y = keras_text_classifier.load_data(['../data/sentiment/samples/positive.txt', '../data/sentiment/samples/negative.txt'], load_method=keras_text_classifier.load_data_from_file)
